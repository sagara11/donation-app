...
test("cannot be initialized twice", async (t) => {
  ...
  await t.throwsAsync(
    alice.call(contract, "init", { beneficiary: beneficiary.accountId })
  );
});
...
test("records the donation", async (t) => {
  ...
  const donation: Donation = await contract.view("get_donation_by_number", {
    donation_number: donation_idx,
  });
